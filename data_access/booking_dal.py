...
class BookingDAL(BaseDAL):
    ...
    def get_by_guest_id(self, guest_id: int, active_only: bool = False) -> list[Booking]:
        if active_only:
            cursor = self.conn.execute("SELECT * FROM Booking WHERE guest_id=? AND status=0", (guest_id,))
        else:
    ...
    def create_booking(self, booking: Booking) -> Booking:
        ...
        cursor = self.conn.execute(
            "INSERT INTO Booking (room_id, guest_id, start_date, end_date, status) VALUES (?, ?, ?, ?, ?)",
    ...
    def cancel_booking(self, booking_id: int) -> bool:
        result = self.conn.execute(
            "UPDATE Booking SET status=1 WHERE id=? AND status=0",
            (booking_id,)
        )
        self.conn.commit()
        return result.rowcount > 0
    ...
    def get_by_date_range(self, start_date, end_date) -> list[Booking]:
        ...
        end_str = end_date.isoformat() if hasattr(end_date, "isoformat") else str(end_date)
        cursor = self.conn.execute(
            "SELECT * FROM Booking WHERE start_date <= ? AND end_date >= ?",
            (end_str, start_str)
        )
        rows = cursor.fetchall()
        return [self.__row_to_booking(row) for row in rows]
